 ...
 * @author Zqq
 */
public final class ImportStatment extends AbstractIncludeStatment {

    private final String[] exportNames;
    private final ResetableValueExpression[] toResetableValues;
    private final boolean exportAll;

    public ImportStatment(Expression templateNameExpr, Expression paramsExpr, String[] exportNames, ResetableValueExpression[] toContextValues, Template template, int line, int column) {
    ...
    public Object execute(final Context context) {
        ...
        if (exportAll) {
            ...
            for (int i = 0, len = vars.length; i < len; i++) {
                StatmentUtil.executeSetValue(myToResetableValues[i], context, vars[i]);
            }
        }
        return null;
    }
