...
public class MethodMatchSupport {

    static <T extends Executable> T findBest(
            List<T> executables, @Nullable Class<?>... args) {
        List<ExecutableAware<T>> wrappers = Lists.collect(executables, ExecutableWrapper::of);
        var result = MethodMatchUtils.findBest(wrappers, MethodMatchUtils::distance, args, 0);
