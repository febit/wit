...
import static org.febit.wit.script.WitTestSupport.WIT;
...
class WitTest {
    ...
    @Test
    void isEnabled() {
        Assertions.assertFalse(WIT.isEnabled(Feature.LOOSE_VAR));
        Assertions.assertTrue(WIT.isEnabled(Feature.TRIM_CODE_BLOCK_BLANK_LINE));
    }
    ...
    void scriptNormalizedCaches() throws NoSuchSourceException {
        ...
        assertNotSame(
                WIT.script("fixtures:/a.wit"),
                WIT.script("fixtures:/x/../a.wit")
        );
        assertNotSame(
                WIT.script("fixtures:/a.wit"),
                WIT.script("fixtures:/x/b.wit", "a.wit")
        );
        Assertions.assertEquals(
                WIT.script("fixtures:/a.wit"),
                WIT.script("fixtures:/x/../a.wit")
    ...
    void scriptIllegal() {
        ...
        assertThrows(NoSuchSourceException.class,
                () -> WIT.script("fixtures:/no-such-path/x", "../../../b.wit")
        );
    }
    ...
    void asOut() {
        ...
        Assertions.assertTrue(out.preferBytes());
        Assertions.assertEquals(WIT.charset(), out.charset());

        out = WIT.asOut(output, StandardCharsets.US_ASCII);
        assertNotNull(out);
        assertInstanceOf(OutputStreamOut.class, out);
        Assertions.assertTrue(out.preferBytes());
        Assertions.assertEquals(StandardCharsets.US_ASCII, out.charset());

        out = WIT.asOut(writer);
        assertNotNull(out);
        assertInstanceOf(WriterOut.class, out);
        Assertions.assertFalse(out.preferBytes());
        Assertions.assertEquals(WIT.charset(), out.charset());
