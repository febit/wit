...
import org.febit.wit.asm.AsmBeanAccessorFactory;
...
public class DefaultAccessorFactory implements AccessorFactory {
    ...
    private <T> Render<T> resolveRender(final Class<?> type) {
        var resolver = mappedRenders.get(type);
        if (resolver != null) {
            return (Render<T>) resolver;
        }
        resolver = lookup(renders, type);
        if (resolver == null) {
            resolver = fallbackFactory.render(type);
        }
        return (Render<T>) mappedRenders.putIfAbsent(type, resolver);
    }
    ...
    public <T> Render<T> render(Class<T> type) {
        var resolver = (Render<T>) this.mappedRenders.unsafeGet(type);
        if (resolver != null) {
            return resolver;
        }
